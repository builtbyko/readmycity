...
PARKING_ROOT = ROOT.parent / "99_sonota" / "00_parking"
...
GTFS_ROOT = Path(os.environ.get("OKAYAMA_GTFS_ROOT", ROOT.parent / "_local_data" / "0kayama_GTFS"))
JINRYU_PATH = Path(os.environ.get("OKAYAMA_JINRYU_CSV", ROOT.parent / "_local_data" / "zinryu_okayama" / "kobetsuarea_time.csv"))
...
def prepare_osm_extra(raw):
    ...
    seen = set()
    private_tag_prefixes = ("addr:", "contact:")
    private_tag_keys = {
        "email",
        "fax",
    ...
    def public_tags(tags):
        return {
            ...
            if key not in private_tag_keys
            and not any(key.startswith(prefix) for prefix in private_tag_prefixes)
        }
...
<div id="layout">
  <aside>
    ...
    <div class="guide">
      ...
      <span class="mini">人流、バス、密度メッシュは必要な時だけONにすると読みやすいです。__BUILDING_AGE_GUIDE__</span>
    </div>
    <div class="actions">
    ...
    <div class="section" style="border-top:0;margin-top:8px;padding-top:0">
      ...
      <p class="note">ONにすると、人流ランキング、商業、歩行者空間、緑地、駐車場、主要交通などを表示します。</p>
    </div>
    ...
    <div class="section">
      <h2>__BUILDING_AGE_SECTION_TITLE__</h2>
      <label class="row" style="__BUILDING_AGE_ROW_STYLE__">
      ...
      <p class="note">__BUILDING_AGE_NOTE__鉄道駅は地図の目印になるよう初期ONにしています。</p>
    </div>
    <div class="section">
      <h2>駐車場・低未利用地</h2>
      <label class="row">
...
const initiallyClosed=new Set([
  "OSMレイヤー",
  "分析補助レイヤー",
  "__BUILDING_AGE_SECTION_TITLE__",
  "密度ポリゴン（約250mメッシュ）",
  "人流（2023年平均）",
  "表示の考え方"
]);
  ...
  return L.geoJSON(FLOW_AREA_APPROX,{
    ...
    onEachFeature:(feature,layer)=>{
      ...
      layer.bindPopup(
        ...
        `<br><span style="color:#687582">正式な125mメッシュ境界ではなく、分析用の参考範囲です。</span>`
      ).bindTooltip(`${p.name||""}: 参考範囲`,{sticky:true});
    }
  });
}
...
html = (
    ...
    .replace(
        "__BUILDING_AGE_SECTION_TITLE__",
        "鉄道駅" if LIGHTWEIGHT else "建物年代・鉄道駅",
    )
    .replace(
        "__BUILDING_AGE_GUIDE__",
        "" if LIGHTWEIGHT else "建物年代も重いので初期OFFです。",
    )
    .replace(
        "__BUILDING_AGE_ROW_STYLE__",
        "display:none" if LIGHTWEIGHT else "",
    )
    .replace(
        "__BUILDING_AGE_NOTE__",
        "" if LIGHTWEIGHT else "建物年代は古い建物を茶色、新しい建物を青系で表示します。重めなので初期OFFです。",
    )
